...
if (!process.env.FighFiveBotToken) {
    console.log('Error: Specify token in environment');
    process.exit(1);
}
if (!process.env.HighFiveBotConfig) {
    process.env.HighFiveBotConfig="highfivebot.json";
}
...
/**
 * load highfivebot configuration
 **/
var highfiveConf = require('./config/'+process.env.HighFiveBotConfig);

/**
 * localisation
 */
String.prototype.__ = function (toreplace) {
    if (typeof highfiveConf.str[this] === "string") {
        if (typeof toreplace === "object") {
            return highfiveConf.str[this].replaceArray(toreplace);
        } else {
            return highfiveConf.str[this];
        }
    } else {
        if (typeof toreplace === "object") {
...
String.prototype.__n = function (numb, toreplace) {
    ...
    toreplace.numb = numb;
    if (typeof highfiveConf.str[this] === "object") {
        if (numb > 1) {
            if (typeof highfiveConf.str[this].multi === "string") {
                return highfiveConf.str[this].multi.__(toreplace);
            } else {
                return this.__(toreplace); //fallback, generaly wrong
            }
        } else {
            if (typeof highfiveConf.str[this].one === "string") {
                return highfiveConf.str[this].one.__(toreplace);
            } else {
 ...
 */
var today = new Date();
var csv_log_file = fs.createWriteStream('./persist/logs/highfiveRewards-'+today.getFullYear()+'-'+(today.getMonth()+1)+'.log.csv', {flags : 'a'});
...
var bot = controller.spawn({
    token: process.env.FighFiveBotToken
}).startRTM();

if (highfiveConf.utterances) {
    bot.utterances.yes = new RegExp("^("+highfiveConf.utterances.yes.join("|")+")","i");
    bot.utterances.no = new RegExp("^("+highfiveConf.utterances.no.join("|")+")","i");
}
...
app.use('/top', function(req, res) {
    ...
    params = {  lists: userTop,
        rewardname : highfiveConf.reward.name.multi
    };
...
var server = app.listen(app.get('port'), function() {
    console.log('highfivebot stats availaible on http://' + server.address().port);
});
...
nextCronDate.setHours(highfiveConf.cronHourUTC, highfiveConf.cronMinuteUTC, 0);
setInterval(function(){
    now = new Date();
    if (now>nextCronDate) {
        highfiveEveryDay(); // update user list and give Gift
...
function highfiveEveryDay() {
    // Change log file if necessary
    if (csv_log_file_month != today.getMonth()+1) {
        csv_log_file = fs.createWriteStream('./persist/logs/highfiveRewards-'+today.getFullYear()+'-'+(today.getMonth()+1)+'.log.csv', {flags : 'a'});
                ...
                (function (m) {
                    controller.storage.users.get(res.members[m].id, function (err, user) {
                        ...
                        if ((res.members[m].deleted) ||
                            (highfiveConf.restrictedToUser.length && highfiveConf.restrictedToUser.indexOf(res.members[m].name) == -1) ||
                            (highfiveConf.excludedUser.length && highfiveConf.excludedUser.indexOf(res.members[m].name) != -1)
                        ) {
                            user.excluded = true;
                        }
                        ...
                        if (!user.excluded) {
                            ...
                            &&  highfiveConf.reward.excludedDays.indexOf(today.getDay())===-1) {
                                // update daily gift counter
                                giftGiven = 0;
                                if (user.gift + highfiveConf.reward.recognitionByDay <= highfiveConf.reward.recognitionMax) {
                                    user.gift = user.gift + highfiveConf.reward.recognitionByDay;
                                    giftGiven = highfiveConf.reward.recognitionByDay;
                                } else {
                                    user.gift = highfiveConf.reward.recognitionMax;
                                    giftGiven = highfiveConf.reward.recognitionMax - user.gift;
                                }
                                bot.api.chat.postMessage({
                                    ...
                                    'text': "Hello {user},\ntoday you've got {gifts} {giftEmoji} to reward your colleague. If you whant to know how many rewards you've got just tell me `hello`.".
                                        __({"user":user.name,"gifts":user.gift,"giftEmoji":highfiveConf.reward.giftEmoji}),
                                    'as_user':true
                                },function(err, user) {
                                ...
                                csvlog(highfiveConf.botName, giftGiven, user.name, "highfiveEveryDay");
                            }
                        } else {
                            user.gift = 0;
                        }
                ...
                (function (m) {
                    controller.storage.channels.get(res.channels[channel].id, function (err, c) {
                        ...
                        if ((res.channels[m].is_archived) ||
                            (highfiveConf.restrictedToUser.length && highfiveConf.restrictedToChannel.indexOf(res.channels[m].name) == -1) ||
                            (highfiveConf.excludedUser.length && highfiveConf.excludedChannel.indexOf(res.channels[m].name) != -1)
                        ) {
                            c.excluded = true;
                        }
 ...
 */
controller.hears(['.*'+highfiveConf.reward.giftEmoji+'.*','.*'+highfiveConf.reward.giftEmojiAlias+'.*'], 'ambient', function(bot, message) {
    ...
    var reg2 = new RegExp(highfiveConf.reward.giftEmoji,'g');
    var rewardcount = (message.text.match(reg2) || []).length;
    // have we an alias with :thanks"
    if (highfiveConf.reward.giftEmoji!=highfiveConf.reward.giftEmojiAlias) {
        var reg3 = new RegExp(highfiveConf.reward.giftEmojiAlias,'g');
        ...
        controller.storage.users.get(message.user, function(err, user) {
            if (user && typeof user.gift != undefined && user.gift>=(rcount*rlist.length)) {
                ...
                bot.api.reactions.add({
                    timestamp: message.ts,
                    channel: message.channel,
                    name: highfiveConf.giftHeardEmoji,
                }, function(err, res) {
                    if (err) {
                        bot.botkit.log('Failed to add emoji reaction :(', err);
                    }
                ...
                controller.storage.users.save(user, function(err,user) {
                    ...
                    for (var who in rlist) {
                        ...
                        controller.storage.users.get(rlist[who], function (err, rewarded) {
                            ...
                            bot.api.chat.postMessage({
                                'channel': rlist[who],
                                'text': "You received {somerewards} from {giver}.".__({"somerewards":rcount + ' ' + highfiveConf.reward.rewardEmoji, "giver": giverName}),
                                'as_user':true
                            });
                    ...
                    bot.api.chat.postMessage({
                        'channel': giverID ,
                        'text': "You gave {someeggs} to {rewarded}".__({"someeggs":(rcount*rlist.length) + ' ' + highfiveConf.reward.rewardEmoji, "rewarded":namelist }),
                        'as_user':true
                    });
                    //bot.reply(message, "Giver "+rcount*rlist.length + ' ' + highfiveConf.reward.rewardEmoji + ' à ' + namelist + 'par <@'+user.id+'>');
                });
            } else {
                // Fail to reward
                bot.api.chat.postMessage({
                    'channel':user.id,
                    'text':  "Sorry, you didn't have enough {egg}, you need {eggs}.".__({"egg":highfiveConf.reward.giftEmoji,"eggs":rewardcount*rewarded.length + ' ' + highfiveConf.reward.giftEmoji}),
                    'as_user':true
                });
                bot.api.reactions.add({
                    timestamp: message.ts,
...
controller.hears(['hello', 'hi', 'hello'.__(), 'hi'.__()], 'direct_message', function(bot, message) {
    ...
    controller.storage.users.get(message.user, function(err, user) {
        if (user && user.name) {
            msgTxt = "Hello {name}!\n".__({'name':user.name });
            if ( user.gift > 0) {
                msgTxt = msgTxt + "You have {rewards} to give today.\n".__({'rewards':user.gift+' '+highfiveConf.reward.giftEmoji});
            } else {
                msgTxt = msgTxt + "You don't have {rewards} to give today. See you tomorow!\n".__({'rewards':highfiveConf.reward.giftEmoji});
            }
            if ( user.reward > 0) {
                msgTxt = msgTxt +  "You have received {rewards}.".__({"rewards":user.reward+' '+highfiveConf.reward.rewardEmoji});
            } else {
                msgTxt = msgTxt +  "You haven't already received {rewards}.".__({"rewards":highfiveConf.reward.rewardEmoji});
            }
...
controller.hears(['uptime'], 'direct_message,', function(bot, message) {
        ...
        bot.reply(message,
            ':'+highfiveConf.giftHeardEmoji+': I am a bot named <@' + bot.identity.name +
             '>. I have been running for ' + uptime + ' on ' + hostname + ".\n"+
                "Configuration : "+process.env.HighFiveBotConfig
        );
    });
...
function giveReward (convo, userId, gift) {
    ...
    controller.storage.users.get(userId, function (err, user) {
        // log reward
        csvlog(user.name, highfiveConf.reward.rewardRL[gift.realid].cost, highfiveConf.botName , "useReward");
        // remove cost
        user.reward = user.reward - highfiveConf.reward.rewardRL[gift.realid].cost;
        controller.storage.users.save(user);
        // Confirm user
        convo.say({
            'text': "Done!".__()  + ' ' + highfiveConf.reward.rewardRL[gift.realid].getinfo,
            'as_user': true,
            "attachments": [gift]
        });
        // Post confirmation in admin channel
        bot.api.chat.postMessage({
            'channel': highfiveConf.adminChanel,
...
function rewardList (user) {
    var attachs = [];
    var line =1;
    for (var rewardKey in highfiveConf.reward.rewardRL) {
        if (user.reward >= highfiveConf.reward.rewardRL[rewardKey].cost) {
            attachs.push({
                "fallback": highfiveConf.reward.rewardRL[rewardKey].desc,
                "color": highfiveConf.reward.rewardRL[rewardKey].color,
                "title": "#" + line + " : " + highfiveConf.reward.rewardRL[rewardKey].desc,
                "thumb_url": highfiveConf.reward.rewardRL[rewardKey].thumb_url,
                "text": "Value:".__() + highfiveConf.reward.rewardRL[rewardKey].cost + " " + highfiveConf.reward.rewardEmoji,
                "realid" : rewardKey
            });
            line++;
        }
